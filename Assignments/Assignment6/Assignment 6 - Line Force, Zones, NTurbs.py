...
def min_dist_to_any_line(x, y):
    """Return the minimum squared distance penalty from (x, y) to the nearest line or road."""
    # Field lines (target distance 0)
    best_field = np.inf
    ...
    pen_field = max(0.0, dist_field - 0.002)

    # Normal roads (target distance 10m = 0.010km)
    best_rn = np.inf
    for (x1, y1), (x2, y2) in road_segments_normal:
        d = _point_to_segment_dist_sq(x, y, x1, y1, x2, y2)
        if d < best_rn: best_rn = d
    dist_rn = np.sqrt(best_rn) if best_rn != np.inf else np.inf
    pen_rn = max(0.0, abs(dist_rn - 0.010) - 0.002)

    # Toftlundvej (target distance 15m = 0.015km)
    best_rt = np.inf
    for (x1, y1), (x2, y2) in road_segments_toft:
        d = _point_to_segment_dist_sq(x, y, x1, y1, x2, y2)
        if d < best_rt: best_rt = d
    dist_rt = np.sqrt(best_rt) if best_rt != np.inf else np.inf
    pen_rt = max(0.0, abs(dist_rt - 0.015) - 0.002)

    best_pen = min(pen_field, pen_rn, pen_rt)
    return best_pen ** 2
...
def _make_objective(n_turbines, fmodel_local):
    ...
    def _batched_objective(batch_pos):
        # batch_pos is shape (n_particles, n_turbines, 2)
        n_particles = batch_pos.shape[0]
        
        # 1. FLORIS Runs (sequential over batch, but fast enough)
        aeps = cp.zeros(n_particles)
        for i in range(n_particles):
            # Extract layout to CPU list of floats for FLORIS
            layout = batch_pos[i]
            ...
            fmodel_local.run()
            aeps[i] = fmodel_local.get_farm_AEP() / 1e9  # Convert to GWh
            
        values = aeps * AEP_WEIGHT
        
        # 2. Penalties (Vectorized over entire batch on GPU)
        px = batch_pos[:, :, 0:1]  # (n_particles, n_turbines, 1)
        py = batch_pos[:, :, 1:2]  # (n_particles, n_turbines, 1)
        
        # Line penalty
        # Function to compute distance to a set of segments
        def _dist_to_segs(px, py, p1, dxdy, lensq):
            if p1 is None:
                return cp.full((n_particles, n_turbines), cp.inf)
            px_diff = px - p1[:, 0]
            py_diff = py - p1[:, 1]
        ...
        dist_line = _dist_to_segs(px, py, l_p1, l_dxdy, l_lensq)
        dist_rn = _dist_to_segs(px, py, rn_p1, rn_dxdy, rn_lensq)
        dist_rt = _dist_to_segs(px, py, rt_p1, rt_dxdy, rt_lensq)
        
        # Deadband: 0 penalty if within 2m (0.002km) of the target
        pen_line = cp.maximum(0.0, dist_line - 0.002)
        pen_rn = cp.maximum(0.0, cp.abs(dist_rn - 0.010) - 0.002)
        pen_rt = cp.maximum(0.0, cp.abs(dist_rt - 0.015) - 0.002)
        
        # The line penalty is the minimum of placing it on field lines, normal roads, or Toftlundvej
        best_pen = cp.minimum(cp.minimum(pen_line, pen_rn), pen_rt)
        line_pen = cp.sum(best_pen**2, axis=1)
        
        # Exclusion penalty
        # batch_pos is (P, T, 2), exc_xy is (E, 2) -> (P, T, E, 2)
        diff_exc = batch_pos[:, :, None, :] - exc_xy[None, None, :, :]
        dist_exc = cp.sqrt(cp.sum(diff_exc**2, axis=3))
        pen_exc = cp.maximum(0, exc_r[None, None, :] - dist_exc)
...
def save_layout_plot(positions, n_turbines, filename, title=""):
    ...
    for x, y in positions:
        ...
        ax.add_patch(plt.Circle((x, y), radius=MIN_TURBINE_SPACING / 2.0, facecolor='none', edgecolor='magenta', linestyle='-', linewidth=1.5, label="Min Spacing" if first_best else "_nolegend_"))
        first_best = False
...
def run_optimization(n_turbines, seed=42, maxiter=50, disp=True):
    ...
    flat_best_pos, best_score = pso.optimize(objective_fn, callback=on_new_best)
    ...
    return flat_best_pos, best_positions, exact_aep
...
def _run_one(n):
    x0, pos, exact_aep = run_optimization(n_turbines=n, maxiter=100)
    ...
    return {"n": n, "x0": x0, "positions": pos, "aep": float(exact_aep), "total_pen": total_pen}
