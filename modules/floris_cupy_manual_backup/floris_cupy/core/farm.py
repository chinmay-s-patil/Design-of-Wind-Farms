...
from floris_cupy.core.rotor_velocity import compute_tilt_angles_for_floating_turbines_map
from floris_cupy.core.turbine.operation_models import POWER_SETPOINT_DEFAULT
...
from floris_cupy.utilities import load_yaml
...
class Farm(BaseClass):
    ...
    def initialize(self, sorted_indices):
        ...
        self.awc_modes_sorted = np_cpu.take_along_axis(
            self.awc_modes,
            sorted_indices[:, :, 0, 0].get() if hasattr(sorted_indices, 'get') else sorted_indices[:, :, 0, 0],
            axis=1,
        )
    ...
    def expand_farm_properties(self, n_findex: int, sorted_coord_indices):
        ...
        self.turbine_type_map_sorted = np_cpu.take_along_axis(
            np_cpu.reshape(
                np_cpu.array([turb["turbine_type"] for turb in self.turbine_definitions] * n_findex),
                np_cpu.shape(sorted_coord_indices)
            ),
            sorted_coord_indices.get() if hasattr(sorted_coord_indices, 'get') else sorted_coord_indices,
            axis=1
        )
    ...
    def finalize(self, unsorted_indices):
        ...
        self.turbine_type_map = np_cpu.take_along_axis(
            self.turbine_type_map_sorted,
            unsorted_indices[:,:,0,0].get() if hasattr(unsorted_indices, 'get') else unsorted_indices[:,:,0,0],
            axis=1
        )
        self.state.USED
