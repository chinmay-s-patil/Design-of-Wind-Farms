...
from floris_cupy.core.wake_velocity.gauss import gaussian_function
from floris_cupy.type_dec import floris_float_type
...
def rCalt(wind_veer, sigma_y, sigma_z, y, y_i, delta_y, delta_z, z, HH, Ct,
    ...
    a = (
        np.cos(wind_veer) ** 2 / (2 * sigma_y ** 2)
        + np.sin(wind_veer) ** 2 / (2 * sigma_z ** 2)
    )
    b = (
        -np.sin(2 * wind_veer) / (4 * sigma_y ** 2)
        + np.sin(2 * wind_veer) / (4 * sigma_z ** 2)
    )
    c = (
        np.sin(wind_veer) ** 2 / (2 * sigma_y ** 2)
        + np.cos(wind_veer) ** 2 / (2 * sigma_z ** 2)
    )
    delta_y = y - y_i - delta_y
    delta_z = z - HH - delta_z
