...
def wake_added_yaw(
    ...
    rT_squared = yLocs ** 2 + zT ** 2
    # This looks like spanwise decay;
    # it defines the vortex profile in the spanwise directions
    core_shape = 1 - np.exp(-rT_squared / (eps ** 2))
    v_top = (Gamma_top * zT) / (2 * pi * rT_squared) * core_shape
    v_top = np.mean( v_top, axis=(2,3) )
    # w_top = (-1 * Gamma_top * yLocs) / (2 * pi * rT) * core_shape * decay

    # bottom vortex
    zB = z_i - (HH - D / 2) + NUM_EPS
    rB_squared = yLocs ** 2 + zB ** 2
    core_shape = 1 - np.exp(-rB_squared / (eps ** 2))
    v_bottom = (Gamma_bottom * zB) / (2 * pi * rB_squared) * core_shape
    v_bottom = np.mean( v_bottom, axis=(2,3) )
    # w_bottom = (-1 * Gamma_bottom * yLocs) / (2 * pi * rB) * core_shape * decay

    # wake rotation vortex
    zC = z_i - HH + NUM_EPS
    rC_squared = yLocs ** 2 + zC ** 2
    core_shape = 1 - np.exp(-rC_squared / (eps ** 2))
    v_core = (Gamma_wake_rotation * zC) / (2 * pi * rC_squared) * core_shape
...
def calculate_transverse_velocity(
    ...
    # it defines the vortex profile in the spanwise directions
    core_shape = 1 - np.exp(-rT_squared / (eps ** 2))
    V1 = (Gamma_top * zT) / (2 * pi * rT_squared) * core_shape * decay
    W1 = (-1 * Gamma_top * yLocs) / (2 * pi * rT_squared) * core_shape * decay

    # bottom vortex
    zB = z - (HH - D / 2) + NUM_EPS
    rB_squared = yLocs ** 2 + zB ** 2
    core_shape = 1 - np.exp(-rB_squared / (eps ** 2))
    V2 = (Gamma_bottom * zB) / (2 * pi * rB_squared) * core_shape * decay
    W2 = (-1 * Gamma_bottom * yLocs) / (2 * pi * rB_squared) * core_shape * decay

    # wake rotation vortex
    zC = z - HH + NUM_EPS
    rC_squared = yLocs ** 2 + zC ** 2
    core_shape = 1 - np.exp(-rC_squared / (eps ** 2))
    V5 = (Gamma_wake_rotation * zC) / (2 * pi * rC_squared) * core_shape * decay
    W5 = (-1 * Gamma_wake_rotation * yLocs) / (2 * pi * rC_squared) * core_shape * decay

    ### Boundary condition - ground mirror vortex

    # top vortex - ground
    zTb = z + (HH + D / 2) + NUM_EPS
    rTb_squared = yLocs ** 2 + zTb ** 2
    # This looks like spanwise decay;
    # it defines the vortex profile in the spanwise directions
    core_shape = 1 - np.exp(-rTb_squared / (eps ** 2))
    V3 = (-1 * Gamma_top * zTb) / (2 * pi * rTb_squared) * core_shape * decay
    W3 = (Gamma_top * yLocs) / (2 * pi * rTb_squared) * core_shape * decay

    # bottom vortex - ground
    zBb = z + (HH - D / 2) + NUM_EPS
    rBb_squared = yLocs ** 2 + zBb ** 2
    core_shape = 1 - np.exp(-rBb_squared / (eps ** 2))
    V4 = (-1 * Gamma_bottom * zBb) / (2 * pi * rBb_squared) * core_shape * decay
    W4 = (Gamma_bottom * yLocs) / (2 * pi * rBb_squared) * core_shape * decay

    # wake rotation vortex - ground effect
    zCb = z + HH + NUM_EPS
    rCb_squared = yLocs ** 2 + zCb ** 2
    core_shape = 1 - np.exp(-rCb_squared / (eps ** 2))
    V6 = (-1 * Gamma_wake_rotation * zCb) / (2 * pi * rCb_squared) * core_shape * decay
    W6 = (Gamma_wake_rotation * yLocs) / (2 * pi * rCb_squared) * core_shape * decay
