        ...
        *,
        ...
        Args:
            ...
            z_locations (np.array): vertical locations in wake
                (not used in Jiménez)
            turbine (:py:class:`floris_cupy.core.turbine.Turbine`):
                Turbine object
            coord
                (:py:meth:`floris_cupy.core.turbine_map.TurbineMap.coords`):
                Spatial coordinates of wind turbine.
            flow_field
                (:py:class:`floris_cupy.core.flow_field.FlowField`):
                Flow field object.
        ...
        delta_x = x - x_i
        A = 15 * (2 * kd * delta_x / rotor_diameter_i + 1) ** 4.0 + xi_init ** 2.0
        B = (30 * kd / rotor_diameter_i)
        B = B * (2 * kd * delta_x / rotor_diameter_i + 1) ** 5.0
        C = xi_init * rotor_diameter_i * (15 + xi_init ** 2.0)
        D = 30 * kd

        yYaw_init = (xi_init * A / B) - (C / D)
        deflection = yYaw_init + ad + bd * delta_x
