...
class AWCTurbine(BaseOperationModel):
    ...
    def power(
        ...
        **_ # <- Allows other models to accept other keyword arguments
        ...
        if not np_cpu.all(np_cpu.isin(awc_modes, valid_entries)):
            raise UserWarning(
        ...
        if np.any(np.isclose(to_cpu(base_powers[helix_mask])/1000,to_cpu(np.max(power_thrust_table['power'])))):
            raise UserWarning(
        ...
        awc_powers = AWCTurbine.AWC_model(
            ...
            np.asarray(awc_amplitudes)[helix_mask]
        )
        powers[helix_mask] = to_cpu(awc_powers)
    ...
    def thrust_coefficient(
        ...
        **_ # <- Allows other models to accept other keyword arguments
        ...
        thrust_coefficients[helix_mask] = to_cpu(awc_thrust_coefficients)
