...
from floris_cupy.heterogeneous_map import HeterogeneousMap
from floris_cupy.type_dec import NDArrayFloat
...
class WindRose(WindDataBase):
    ...
    def __init__(
        ...
        wind_directions = np.asarray(wind_directions)
        if wind_directions.ndim != 1:
            raise TypeError("wind_directions must be a 1D array")

        wind_speeds = np.asarray(wind_speeds)
        if wind_speeds.ndim != 1:
            raise TypeError("wind_speeds must be a 1D array")
        ...
        # Check ti_table is a float or an array
        if not isinstance(ti_table, (float, np.ndarray, np_cpu.ndarray)):
            raise TypeError("ti_table must be a float or an array")
        ...
        if value_table is not None:
            ...
            self.value_table = np.asarray(value_table)
        else:
    ...
    def plot(
        ...
        **Note**: Based on code provided by Patrick Murphy from the University
        ...
        wd_bins = np_cpu.asarray(wind_rose_aggregate.wind_directions.get() if hasattr(wind_rose_aggregate.wind_directions, 'get') else wind_rose_aggregate.wind_directions)
        ws_bins = np_cpu.asarray(wind_rose_aggregate.wind_speeds.get() if hasattr(wind_rose_aggregate.wind_speeds, 'get') else wind_rose_aggregate.wind_speeds)
        freq_table = np_cpu.asarray(wind_rose_aggregate.freq_table.get() if hasattr(wind_rose_aggregate.freq_table, 'get') else wind_rose_aggregate.freq_table)
        ...
        norm_ws = mpl.colors.Normalize(vmin=np_cpu.min(ws_bins), vmax=np_cpu.max(ws_bins))
            ...
            for ws_idx, ws in reversed(list(enumerate(ws_bins))):
                ...
                rects.append(
                    ax.bar(
                        np_cpu.radians(wd),
                        plot_val,
                        width=0.9 * np_cpu.radians(wd_step),
        ...
        ax.set_theta_offset(np_cpu.pi / 2.0)
        ax.set_theta_zero_location("N")
        ax.set_xticks(np_cpu.arange(0, 2 * np_cpu.pi, np_cpu.pi / 4))
    ...
    def plot_ti_over_ws(
        ...
        ax.plot(np_cpu.asarray(self.ws_flat.get() if hasattr(self.ws_flat, 'get') else self.ws_flat),
                np_cpu.asarray((self.ti_table_flat * 100).get() if hasattr(self.ti_table_flat, 'get') else self.ti_table_flat * 100),
    ...
    def plot_value_over_ws(
        ...
        ax.plot(np_cpu.asarray(self.ws_flat.get() if hasattr(self.ws_flat, 'get') else self.ws_flat),
                np_cpu.asarray(self.value_table_flat.get() if hasattr(self.value_table_flat, 'get') else self.value_table_flat),
                marker=marker, ls=ls, color=color)
        ax.set_xlabel("Wind Speed (m/s)")
        ax.set_ylabel("Value")
        ax.grid(True)
...
class WindTIRose(WindDataBase):
    ...
    def __init__(
        ...
        turbulence_intensities = np.asarray(turbulence_intensities)
        if turbulence_intensities.ndim != 1:
            raise TypeError("turbulence_intensities must be a 1D array")
    ...
    def plot(
        ...
        **Note**: Based on code provided by Patrick Murphy from the University
        ...
        if wind_rose_var == "ws":
            ...
            var_bins = np_cpu.asarray(wind_rose_aggregated.wind_speeds.get() if hasattr(wind_rose_aggregated.wind_speeds, 'get') else wind_rose_aggregated.wind_speeds)
            freq_table = np_cpu.asarray(wind_rose_aggregated.freq_table.sum(2).get() if hasattr(wind_rose_aggregated.freq_table, 'get') else wind_rose_aggregated.freq_table.sum(2))  # sum along TI dimension
        else:  # wind_rose_var == "ti"
            if wind_rose_var_step is None:
                wind_rose_var_step = 0.04
            wind_rose_aggregated = self.downsample(wd_step, ti_step=wind_rose_var_step)
            var_bins = np_cpu.asarray(wind_rose_aggregated.turbulence_intensities.get() if hasattr(wind_rose_aggregated.turbulence_intensities, 'get') else wind_rose_aggregated.turbulence_intensities)
            freq_table = np_cpu.asarray(wind_rose_aggregated.freq_table.sum(1).get() if hasattr(wind_rose_aggregated.freq_table, 'get') else wind_rose_aggregated.freq_table.sum(1))  # sum along wind speed dimension

        wd_bins = np_cpu.asarray(wind_rose_aggregated.wind_directions.get() if hasattr(wind_rose_aggregated.wind_directions, 'get') else wind_rose_aggregated.wind_directions)
        ...
        norm_wv = mpl.colors.Normalize(vmin=np_cpu.min(var_bins), vmax=np_cpu.max(var_bins))
    ...
    def plot_ti_over_ws(
        ...
        ax.plot(np_cpu.asarray(self.wind_speeds.get() if hasattr(self.wind_speeds, 'get') else self.wind_speeds),
                np_cpu.asarray((mean_ti_values * 100).get() if hasattr(mean_ti_values, 'get') else mean_ti_values * 100),
                marker=marker, ls=ls, color=color)
        ax.set_xlabel("Wind Speed (m/s)")
    ...
    def plot_value_over_ws(
        ...
        ax.plot(np_cpu.asarray(self.wind_speeds.get() if hasattr(self.wind_speeds, 'get') else self.wind_speeds),
                np_cpu.asarray(self.values.get() if hasattr(self.values, 'get') else self.values),
                marker=marker, ls=ls, color=color)
        ax.set_xlabel("Wind Speed (m/s)")
        ax.set_ylabel("Value")
        ax.grid(True)
...
class TimeSeries(WindDataBase):
    ...
    def __init__(
        ...
        # of floats
        if not isinstance(wind_directions, (float, int)):
            wind_directions = np.asarray(wind_directions)
            if wind_directions.ndim != 1:
                raise TypeError("wind_directions must be a float or a 1D array")
        if not isinstance(wind_speeds, (float, int)):
            wind_speeds = np.asarray(wind_speeds)
            if wind_speeds.ndim != 1:
                raise TypeError("wind_speeds must be a float or a 1D array")
        if not isinstance(turbulence_intensities, (float, int)):
            turbulence_intensities = np.asarray(turbulence_intensities)
            if turbulence_intensities.ndim != 1:
                raise TypeError("turbulence_intensities must be a float or a 1D array")

        # At least one of wind_directions, wind_speeds, or turbulence_intensities must be an array
        if (
            isinstance(wind_directions, (float, int))
            and isinstance(wind_speeds, (float, int))
            and isinstance(turbulence_intensities, (float, int))
        ):
            raise TypeError(
                "At least one of wind_directions, wind_speeds, or "
                " turbulence_intensities must be an array"
            )

        # For each of wind_directions, wind_speeds, and turbulence_intensities provided as
        # an array, confirm they are the same length
        if not isinstance(wind_directions, (float, int)) and not isinstance(wind_speeds, (float, int)):
            if len(wind_directions) != len(wind_speeds):
                raise ValueError(
                    "wind_directions and wind_speeds must be the same length if provided as arrays"
                )

        if not isinstance(wind_directions, (float, int)) and not isinstance(
            turbulence_intensities, (float, int)
        ):
        ...
        if not isinstance(wind_speeds, (float, int)) and not isinstance(turbulence_intensities, (float, int)):
        ...
        if isinstance(wind_directions, (float, int)):
            if not isinstance(wind_speeds, (float, int)):
                wind_directions = np.full(len(wind_speeds), float(wind_directions))
            elif not isinstance(turbulence_intensities, (float, int)):
                wind_directions = np.full(len(turbulence_intensities), float(wind_directions))

        if isinstance(wind_speeds, (float, int)):
            if not isinstance(wind_directions, (float, int)):
                wind_speeds = np.full(len(wind_directions), float(wind_speeds))
            elif not isinstance(turbulence_intensities, (float, int)):
                wind_speeds = np.full(len(turbulence_intensities), float(wind_speeds))

        if isinstance(turbulence_intensities, (float, int)):
            if not isinstance(wind_directions, (float, int)):
                turbulence_intensities = np.full(len(wind_directions), float(turbulence_intensities))
            elif not isinstance(wind_speeds, (float, int)):
                turbulence_intensities = np.full(len(wind_speeds), float(turbulence_intensities))
    ...
    def to_WindRose(self, wd_step=2.0, ws_step=1.0, wd_edges=None, ws_edges=None, bin_weights=None):
        ...
        df = pd.DataFrame(
            {
                "wd": np_cpu.asarray(wind_directions_wrapped.get() if hasattr(wind_directions_wrapped, 'get') else wind_directions_wrapped),
                "ws": np_cpu.asarray(self.wind_speeds.get() if hasattr(self.wind_speeds, 'get') else self.wind_speeds),
                "freq_val": np_cpu.ones(len(wind_directions_wrapped)),
            }
        )
        ...
        if bin_weights is not None:
            bin_weights_cpu = np_cpu.asarray(bin_weights.get() if hasattr(bin_weights, 'get') else bin_weights)
            df = df.assign(freq_val=df["freq_val"] * bin_weights_cpu)

        # Add turbulence intensities to dataframe
        ti_cpu = np_cpu.asarray(self.turbulence_intensities.get() if hasattr(self.turbulence_intensities, 'get') else self.turbulence_intensities)
        df = df.assign(turbulence_intensities=ti_cpu)

        # If values is not none, add to dataframe
        if self.values is not None:
            values_cpu = np_cpu.asarray(self.values.get() if hasattr(self.values, 'get') else self.values)
            df = df.assign(values=values_cpu)

        wd_edges_cpu = np_cpu.asarray(wd_edges.get() if hasattr(wd_edges, 'get') else wd_edges)
        ws_edges_cpu = np_cpu.asarray(ws_edges.get() if hasattr(ws_edges, 'get') else ws_edges)
        wd_centers_cpu = np_cpu.asarray(wd_centers.get() if hasattr(wd_centers, 'get') else wd_centers)
        ws_centers_cpu = np_cpu.asarray(ws_centers.get() if hasattr(ws_centers, 'get') else ws_centers)

        # Bin wind speed and wind direction and then group things up
        df = (
            df.assign(
                wd_bin=pd.cut(
                    df.wd, bins=wd_edges_cpu, labels=wd_centers_cpu, right=False, include_lowest=True
                )
            )
            .assign(
                ws_bin=pd.cut(
                    df.ws, bins=ws_edges_cpu, labels=ws_centers_cpu, right=False, include_lowest=True
                )
            )
            .drop(["wd", "ws"], axis=1)
        )
        ...
        wd_cat = CategoricalDtype(categories=wd_centers_cpu, ordered=True)
        ws_cat = CategoricalDtype(categories=ws_centers_cpu, ordered=True)
