...
from floris_cupy.core.turbine import (
    AWCTurbine,
...
def _select_multidim_condition(
    ...
    nearest_conditions, md_map = np_cpu.unique(nearest_conditions, axis=0, return_inverse=True)
...
def power(
    ...
    turb_types = np_cpu.unique(turbine_type_map)
        ...
        if "thrust_coefficient" in turbine_power_thrust_tables[turb_type]:  # Not multidimensional
            ...
            for i, md_cond in enumerate(md_conditions):
                ...
                thrust_coefficient += (
                    thrust_coefficient_functions[turb_type](**thrust_model_kwargs)
                    * np.asarray(turbine_type_map == turb_type)
                    * np.asarray(md_conditions_map[:, None] == i)
                )
